...
func (uc *StoresUsecase) UpdateStore(ctx context.Context, dto data.StoreDto) (*ent.Store, error) {
	...
	if err != nil {
		if ent.IsNotFound(err) {
			return nil, stores_v1.ErrorNotFound("store not found")
		}
		return nil, err
	}
	return s, nil
...
func (uc *StoresUsecase) DeleteStore(ctx context.Context, id int64, tenantID int64) error {
	...
	if err != nil {
		if ent.IsNotFound(err) {
			return stores_v1.ErrorNotFound("store not found")
		}
		return err
	}
	return nil
...
func (uc *StoresUsecase) GetStoresByIDs(ctx context.Context, ids []int64) ([]*ent.Store, error) {
	return uc.repo.GetStoresByIDs(ctx, ids)
}
