...
func (s *StoresService) GetStoresByCoordinates(ctx context.Context, req *v1.GetStoresByCoordinatesRequest) (*v1.GetStoresByCoordinatesReply, error) {
	stores, err := s.su.GetStoresByIDs(ctx, req.GetStoreIds())
	if err != nil {
		return nil, err
	}

	result := make([]*v1.StoreCoordinate, 0, len(stores))
	for _, st := range stores {
		coord := &v1.StoreCoordinate{
			Id:   st.ID,
			Name: st.Name,
		}
		if st.Lat != nil {
	...
	return &v1.GetStoresByCoordinatesReply{
		Stores: result,
	}, nil
}
